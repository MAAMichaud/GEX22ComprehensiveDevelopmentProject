...
void Actor::walk(sf::Vector2f velocity)
{
	if (state == State::Idle) {
		state = State::Walk;
...
void Actor::walk(float vx, float vy)
{
	walk(sf::Vector2f(vx, vy));
}
...
void Actor::setState(State _state)
{
	state = _state;
	animations[state].restart();
}
...
void Actor::updateStates()
{
	...
	/*
	if (state == State::Idle && length(getVelocity()) > 500.1f)
		state = State::Walk;
	*/
}
...
void Actor::updateCurrent(sf::Time dt, CommandQueue& commands)
{
	...
	if (state != State::Dead)
	{
		if (direction == Direction::Left && getVelocity().x > 0)
			direction = Direction::Right;
